# Persona 3 Portable - Tradução (BETA)

## AVISO: O projeto está pausado e não receberá atualizações.

Tradução BETA de **Persona 3 Portable** (Switch) para Português do Brasil, realizada pela **Equipe Atlantis**, feita de fã para fã, de forma 100% humana.

A tradução **NÃO** está completa, você pode encontrar erros, bugs, crashes e outros durante a sua jogatina.
A tradução foi testada e confirmamos que ela funciona do início ao fim, em ambas as rotas, com muitos poucos problemas.
Cerca de 99% do jogo está traduzido e apenas 50% revisado.

---

# 📥 DOWNLOAD

## 👉 [CLIQUE AQUI PARA BAIXAR](https://github.com/Hinrong/P3P-Traduzido-Switch/releases/tag/patch) 👈
*(Certifique-se de baixar o arquivo `P3P_Switch_PTBR_Atlantis.zip` na seção "Assets", e não o zip do código)*

---

# 🛠️ Como Instalar

### Emulador:
Selecione o jogo e aperte o botão direito,
selecione "Open Mod Data Location" ou qualquer
opção relacionada a abrir pasta de mods.
Jogue a pasta P3P_PTBR_Atlantis de dentro do zip
para dentro da pasta que foi aberta.

### Switch:
No seu Cartão SD COM O ATMOSPHERE, abra a pasta do
atmosphere/contents, crie uma pasta com o nome do
ID do jogo. (Exemplo: 010062B01525C000)
Após isso, jogue as pasta de dentro do P3P_PTBR_Atlantis
(ROMFs e EXEFs) de dentro do zip para dentro da pasta criada. 

Créditos pelo tutorial: Phantasie Translante.

# 👥Créditos

## Equipe Atlantis
- 💬 Discord para reportar erros ou bater papo: discord.gg/tM7QZDnWgv
- 🌐 Twitter: https://x.com/AtlantisP3P

## Phantasie Translate
- Pela parceria, ajuda e apoio no projeto.
- 🌐 Site: https://phantasietranslate.com.br/

## Port para Switch
- Ahtheerr

## Líderes do Projeto
- Hinrong (2024-2026)
- em.i (2022-2023)

## Tradutores
- Hinrong
- em.I
- SorinhaPhantasie
- PedroGdP
- Ahtheerr
- GustavoEmiya
- Lursga
- lpedrao
- Rivers
- Jvjfe
- Kami
- Nevaska
- rafilks
- Justine
- ANFenix2005

## Revisores
- Hinrong
- em.I
- PedroGdP
- Jvjfe
- GustavoEmiya

## Beta-Testers
- porcao1214
- Nathan Weiss Belmont
- JamesEdw
- Lorde Guigas
- Panamericano
- Kall

## Designers Gráficos
- Zeros7
- Yuki-sensei
- Jau
- Hinrong

## Hacking
- Ahtheerr
- Hinrong
- PHSticks
- Zeros7
- em.I
